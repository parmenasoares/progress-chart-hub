...
export function Agenda({ patients, onUpdatePatient }: AgendaProps) {
  ...
  useEffect(() => {
    ...
    const savedCalendlyToken = localStorage.getItem(CALENDLY_TOKEN_KEY);
    const savedCalendlyUserUri = localStorage.getItem(CALENDLY_USER_URI_KEY);
  ...
  const handleCalendlyUserUriChange = (value: string) => {
    setCalendlyUserUri(value);
    localStorage.setItem(CALENDLY_USER_URI_KEY, value);
  };

  const syncCalendlyEvents = async () => {
    if (!calendlyToken || !calendlyUserUri) {
      toast({
        title: 'Configure o Calendly',
        description: 'Preencha token pessoal e URI de usuário do Calendly.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      minStartTime.setDate(minStartTime.getDate() - 15);
      ...
      const collection = (scheduledEventsData.collection || []) as Array<{
        name: string;
        start_time: string;
        uri: string;
      }>;
      ...
      for (const event of collection) {
        ...
        for (const invitee of invitees) {
          ...
          const alreadyExists = matchedPatient.sessions.some(
      ...
      toast({
        ...
        description: `${syncedCount} consulta(s) adicionada(s) na agenda.`,
      });
    } catch (error) {
      console.error(error);
      toast({
        title: 'Erro ao sincronizar Calendly',
        description: 'Verifique token/usuário e tente novamente.',
        variant: 'destructive',
      });
    } finally {
  ...
  const connectGoogleCalendar = async () => {
    ...
    try {
      ...
      localStorage.setItem(GOOGLE_CALENDAR_EMAIL_KEY, connectedEmail);
  ...
  useEffect(() => {
    ...
    const alreadyNotified = localStorage.getItem(GOOGLE_NOTIFICATION_DAY_KEY) === todayKey;
    ...
    localStorage.setItem(GOOGLE_NOTIFICATION_DAY_KEY, todayKey);
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex w-full flex-wrap items-center gap-2 lg:w-auto lg:justify-end">
            ...
            <Input
              ...
              onChange={(e) => handleCalendlyUserUriChange(e.target.value)}
              placeholder="Calendly User URI"
              className="w-full bg-card sm:w-72"
            />
            <Input
              value={calendlyToken}
            ...
            />
            <Button variant="outline" size="sm" onClick={syncCalendlyEvents} disabled={isSyncingCalendly} className="w-full sm:w-auto">
              <RefreshCcw className="h-4 w-4" />
              {isSyncingCalendly ? 'Sincronizando Calendly...' : 'Sincronizar Calendly'}
            </Button>
