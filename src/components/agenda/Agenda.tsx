...
export function Agenda({ patients, onUpdatePatient, userId }: AgendaProps) {
  ...
  const handleCalendlyUserUriChange = (value: string) => {
    ...
    localStorage.setItem(storageKey(CALENDLY_USER_URI_KEY), value);
  };
  ...
  const syncCalendlyEventsToCrm = async () => {
    if (!calendlyToken || !calendlyUserUri) {
      toast({
        ...
        description: 'Informe token e URI do usuário para buscar os eventos no CRM.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${syncedCount} consulta(s) puxada(s) para o CRM.`,
      });
    } catch (error) {
      console.error(error);
      toast({
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex w-full flex-wrap items-center gap-2 lg:w-auto lg:justify-end">
            ...
            <Input
              ...
              placeholder="Calendly personal token"
              className="w-full bg-card sm:w-72"
            />
            <Button
              variant="outline"
            ...
            >
              ...
              {isSyncingCalendly ? 'Sincronizando Calendly...' : 'Puxar Calendly → CRM'}
            </Button>
