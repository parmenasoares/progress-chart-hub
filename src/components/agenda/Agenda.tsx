...
const matchPatientByInvitee = (invitee: CalendlyInvitee, patient: Patient) => {
  if (invitee.email && patient.email && normalizeText(invitee.email) === normalizeText(patient.email)) {
    return true;
  }

  if (invitee.name && normalizeText(invitee.name) === normalizeText(patient.name)) {
    return true;
  }
...
export function Agenda({ patients, onUpdatePatient, onAddPatient, userId }: AgendaProps) {
  ...
  const fetchCalendlyCollection = async <T,>(url: string, token: string) => {
    const collection: T[] = [];
    ...
    while (nextPageUrl) {
      ...
      if (!response.ok) {
        throw new Error('Falha ao carregar dados do Calendly.');
      }

      const data = await response.json();
      collection.push(...((data.collection || []) as T[]));

      const nextPage = data.pagination?.next_page as string | null | undefined;
      nextPageUrl = nextPage || null;
    }
  ...
  const syncCalendlyEventsToCrm = async () => {
    ...
    try {
      ...
      minStartTime.setDate(minStartTime.getDate() - 90);

      const maxStartTime = new Date();
      maxStartTime.setDate(maxStartTime.getDate() + 365);
      ...
      const collection = await fetchCalendlyCollection<CalendlyScheduledEvent>(
      ...
      let matchedCount = 0;
      let createdCount = 0;
      const patientsToUpdate = new Map<string, Patient>();
      const patientsToCreate = new Map<string, Partial<Patient>>();

      for (const event of collection) {
        const invitees = await fetchCalendlyCollection<CalendlyInvitee>(`${event.uri}/invitees?count=100`, calendlyToken);

        for (const invitee of invitees) {
          const sessionDate = format(parseISO(event.start_time), 'yyyy-MM-dd');
          const sessionNote = `[Calendly] ${event.name}${invitee.email ? ` · ${invitee.email}` : ''}`;
          const matchedPatient = patients.find((patient) => matchPatientByInvitee(invitee, patient));

          if (matchedPatient) {
            matchedCount += 1;

            const currentPatient = patientsToUpdate.get(matchedPatient.id) || matchedPatient;
            const exists = currentPatient.sessions.some(
            ...
            patientsToUpdate.set(matchedPatient.id, {
              ...currentPatient,
              sessions: [...currentPatient.sessions, newSession],
              completedSessions: currentPatient.completedSessions + 1,
          ...
          const identityKey = normalizeText(invitee.email || invitee.name || `${event.uri}-${event.start_time}`);
          const currentDraft = patientsToCreate.get(identityKey);
          ...
          if (currentDraft) {
            ...
            patientsToCreate.set(identityKey, {
              ...currentDraft,
              sessions: [...currentSessions, newSession],
              sessionHistory: [...(currentDraft.sessionHistory || []), sessionDate],
              completedSessions: (currentDraft.completedSessions || 0) + 1,
            });
          ...
          const inviteeEmail = (invitee.email || '').trim();

          patientsToCreate.set(identityKey, {
            name: inviteeName || 'Lead Calendly',
            phone: 'Não informado',
            ...
            quickContext: `Importado automaticamente do Calendly (${event.name}).`,
      ...
      for (const draft of patientsToCreate.values()) {
        await onAddPatient(draft);
        createdCount += 1;
      }

      toast({
        title: 'Calendly sincronizado',
        description: `${syncedCount} consulta(s) importada(s). Eventos: ${collection.length} · Convites vinculados: ${matchedCount} · Novos no CRM: ${createdCount}.`,
      });
    } catch (error) {
      console.error(error);
      toast({
